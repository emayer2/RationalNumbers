...
public final class RatTerm {
  ...
  public boolean isNaN() {
    return coeff.equals(NaN.getCoeff());
  }
  ...
  public boolean isZero() {
    return coeff.equals(ZERO.getCoeff());
  }
  ...
  public RatTerm negate() {
	// Return NaN is this RatTerm is NaN
	if (coeff.equals(NaN.getCoeff())) {
	  return this;
    } else {
  ...
  public RatTerm add(RatTerm arg) {
	// Returns NaN if either coeff is NaN
	if (coeff.equals(NaN.getCoeff()) || arg.getCoeff().equals(NaN.getCoeff())) {
		return NaN;
	} else if (this.expt != arg.getExpt() && 
			   !arg.getCoeff().equals(ZERO.getCoeff()) && 
	           !this.coeff.equals(ZERO.getCoeff())) {
      // Incompatible exponents
	  throw new IllegalArgumentException("Exponents not compatible: "
			  + this + ", " + arg);
	}
	
	// Tests to see if one of the exponents is zero
	int exp;
    if (this.expt != 0 && arg.getExpt() == 0) {
	  exp = this.expt;
    } else if (this.expt == 0 && arg.getExpt() != 0) {
	  exp = arg.getExpt();
    } else {
      exp = this.expt;
    }
	
	// Calculate the new coefficient, and return
	RatNum newCoeff = this.coeff.add(arg.getCoeff());
	return new RatTerm(newCoeff, exp);
  }
  ...
  public RatTerm sub(RatTerm arg) {
	...
	// Calculate the new coefficient, and return
	RatNum newCoeff = this.coeff.sub(arg.getCoeff());
	return new RatTerm(newCoeff, exp);
  }
  ...
  public RatTerm mul(RatTerm arg) {
	// Returns NaN if either coeff is NaN
	if (this.equals(NaN) || arg.equals(NaN)) {
	  return NaN;
	}
	
	// Calculate the new coefficient/exponent, and return
	RatNum newCoeff = this.coeff.mul(arg.getCoeff());
	int newExpt = this.expt + arg.getExpt();
	return new RatTerm(newCoeff, newExpt);
  }
  ...
  public RatTerm div(RatTerm arg) {
	// Returns NaN if either coeff is NaN, or if the arg is zero
	if (arg.getCoeff().equals(ZERO.getCoeff()) || this.equals(NaN) || arg.equals(NaN)) {
	  return NaN;
	}
	
	// Calculate the new coefficient/exponent, and return
	RatNum newCoeff = this.coeff.div(arg.getCoeff());
	int newExpt = this.expt - arg.getExpt();
	return new RatTerm(newCoeff, newExpt);
  }
  ...
  public RatTerm differentiate() {
	...
	RatNum newCoeff = coeff.mul(new RatNum(expt));
	
	// Subtract one from the exponent
	int newExpt = expt - 1;
	
	// Returns the new RatTerm
	return new RatTerm(newCoeff, newExpt);
  }
  ...
  public RatTerm antiDifferentiate() {
	...
	// Multiply by the exponent
	RatNum newCoeff = coeff.div(new RatNum(expt + 1));
