...
public class Box implements Iterable<Ball> {
    ...
    // Holds the max amount of volume the box can hold
    private double maxVolume;
    
    // Used to compare Ball objects
    private static Comparator<Ball> comp = new Comparator<Ball>() {
    	// Defines the compare() method, which allows Comparator
    	// to actually compare two Ball objects together.
    	public int compare(Ball ba1, Ball ba2) {
    		double vol1 = ba1.getVolume();
    		double vol2 = ba2.getVolume();
    		if (vol1 < vol2) {
    			return -1;
    		} else if (vol1 == vol2){
    			return 0;
    ...
    public boolean add(Ball b) {
    	...
    	// Adds the ball is there is space for it
    	boolean added = false;
    	double bVol = b.getVolume();
    	if ((getVolume() + bVol) <= this.maxVolume) {
    		added = ballContainer.add(b);
    	}
    	return added;
    }
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	...
    	// Gets an iterator for the list to be sorted,
    	...
    	// Sorts the list using the Collections library
